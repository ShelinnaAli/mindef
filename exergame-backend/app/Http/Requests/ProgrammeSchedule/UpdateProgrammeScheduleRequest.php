...
class UpdateProgrammeScheduleRequest extends FormRequest
{
    /**
    ...
    {
        return [
            'programmeId' => 'sometimes|exists:programmes,id',
            'trainerId' => 'sometimes|exists:users,id',
        ...
        if ($startTime && $endTime && $programmeId && ! $validator->errors()->has(['startTime', 'endTime', 'programmeId'])) {
            try {
                ...
                if ($programme) {
                    ...
                    // Validate minimum session duration (at least 1 minute)
                    if ($sessionDurationMinutes < 15) {
                        $validator->errors()->add('endTime', 'Session duration must be at least 15 minute.');
    ...
    private function validateUniqueSchedule($validator, $data)
    {
        ...
        if ($trainerId && $roomId && $day && $startTime && ! $validator->errors()->has(['trainerId', 'roomId', 'day', 'startTime'])) {
            ...
            $scheduleId = $scheduleId ?? $this->route('id');
